...
- (id)initImagePicker
{
    AlbumPickerController *albumPicker = [[AlbumPickerController alloc] initWithStyle:UITableViewStylePlain];
...
- (AlbumPickerController *)albumPicker
{
...
- (void)cancelImagePicker
{
	if ([_imagePickerDelegate respondsToSelector:@selector(ImagePickerControllerDidCancel:)]) {
		[_imagePickerDelegate performSelector:@selector(ImagePickerControllerDidCancel:) withObject:self];
	}
}
...
- (void)selectedAssets:(NSArray *)assets
{
	...
	if (_imagePickerDelegate != nil && [_imagePickerDelegate respondsToSelector:@selector(ImagePickerController:didFinishPickingMediaWithInfo:)]) {
		[_imagePickerDelegate performSelector:@selector(ImagePickerController:didFinishPickingMediaWithInfo:) withObject:self withObject:returnArray];
	} else {
...
- (BOOL)onOrder
{
    return [[Console mainConsole] onOrder];
}

- (void)setOnOrder:(BOOL)onOrder
{
    [[Console mainConsole] setOnOrder:onOrder];
}
