...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[Console mainConsole] removeAllIndex];
...
- (void)preparePhotos
{
    @autoreleasepool {
        ...
        [self.assetGroup enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            ...
            ALAsset *Asset = [[ALAsset alloc] initWithAsset:result];
            [ALAsset setParent:self];
            
            BOOL isAssetFiltered = NO;
            if (self.assetPickerFilterDelegate &&
               [self.assetPickerFilterDelegate respondsToSelector:@selector(assetTablePicker:isAssetFilteredOut:)])
            {
                isAssetFiltered = [self.assetPickerFilterDelegate assetTablePicker:self isAssetFilteredOut:(Asset*)ALAsset];
            }
...
{	
    ...
    if ([[Console mainConsole] onOrder]) {
...
- (BOOL)shouldSelectAsset:(Asset *)asset
{
    ...
    for (Asset *Asset in self.Assets) {
        if (Asset.selected) selectionCount++;
    }
...
- (NSArray *)assetsForIndexPath:(NSIndexPath *)path
{
    ...
    long length = MIN(self.columns, [self.Assets count] - index);
